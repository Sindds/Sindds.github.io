    ...
    <section className='relative min-h-screen flex flex-col justify-center items-center px-4 py-16 sm:py-20 overflow-hidden'>
      ...
      <div className='text-center w-full max-w-xs sm:max-w-lg md:max-w-2xl lg:max-w-3xl xl:max-w-4xl mx-auto mb-12 md:mb-16'>
        <h1 className='text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold mb-4 sm:mb-6 leading-tight'>
          Привет! Я{' '}
          <span className='text-blue-600'>Синдяев Дмитрий</span>,
          <br />
          фронтенд-разработчик
        </h1>

        <p className='text-base sm:text-lg md:text-xl text-gray-600 mb-6 sm:mb-8 leading-relaxed px-2 sm:px-0'>
          Специализируюсь на создании современных, адаптивных и высокопроизводительных
        ...
        <div className='flex flex-col min-[400px]:flex-row justify-center gap-3 sm:gap-4'>
          <NavLink
            to={Page.PORTFOLIO}
            className='bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg transition-all duration-300 transform hover:-translate-y-1 text-sm sm:text-base'
          >
            Мои проекты
          </NavLink>
          <NavLink
            to={Page.CONTACTS}
            className='border border-gray-300 hover:border-blue-600 text-gray-700 hover:text-blue-600 px-6 py-3 rounded-lg transition-all duration-300 transform hover:-translate-y-1 text-sm sm:text-base'
          >
            Написать мне
          </NavLink>
        </div>
        ...
        {technologies.map(tech => (
          <div key={tech.name} className='group relative flex items-center justify-center'>
            <div className='bg-white p-2 rounded-full shadow-lg border border-gray-100 transition-all duration-300 group-hover:scale-110 group-hover:shadow-xl'>
              <img src={tech.icon} alt={tech.name} className='h-7 w-7 lg:h-8 lg:w-8 object-contain' />
            </div>
            <span className='absolute right-full mr-3 px-2 py-1 bg-gray-800 text-white text-xs lg:text-sm rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap'>
              {tech.name}
            </span>
          </div>
        ))}
